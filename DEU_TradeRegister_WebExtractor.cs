      ...
      try
      {
        ...
        foreach (IWebElement row in rows)
        {
          try
          {
            ...
            // Get status - look for status in its specific table cell
            // The status is typically in a td with class containing "statusSuchErgebnisse" or similar
            var statusCellElements = row.FindElements(By.XPath(".//td[contains(@class, 'statusSuchErgebnisse')]//span"));
            string status = "";
            if (statusCellElements.Count > 0)
            {
              status = statusCellElements[0].Text.Trim();
            }
            else
            {
              // Fallback: try to find status by looking for all verticalText spans
              var allVerticalTexts = row.FindElements(By.XPath(".//span[@class='verticalText' or @class='verticalText ']"));
              // Typically: first is city, second is status
              if (allVerticalTexts.Count > 1)
              {
                status = allVerticalTexts[1].Text.Trim();
              }
              else if (allVerticalTexts.Count == 1 && !string.IsNullOrEmpty(city))
              {
                // If we already have city, this might be status
                status = allVerticalTexts[0].Text.Trim();
              }
            }
